...
export class BaselineManager extends EventEmitter {
     ...
     */
    async recordBaseline(testType, testConfig, metrics, options = {}) {
     ...
     */
    async compareToBaseline(testType, testConfig, currentMetrics, options = {}) {
        let baseline;
     ...
     */
    getBaselineHistory(testType, limit = 10) {
     ...
     */
    getPerformanceTrends(testType, timespan = 30 * 24 * 60 * 60 * 1000 // 30 days
    ) {
     ...
     */
    async exportBaselines(outputPath, options = {}) {
        ...
        if (options.dateRange) {
            baselines = baselines.filter(b => b.timestamp >= options.dateRange.start &&
     ...
     */
    async importBaselines(inputPath) {
     ...
     */
    async generateComparisonReport(comparison, outputPath) {
     ...
     */
    async removeBaseline(baselineId) {
     ...
     */
    generateBaselineId(testType, testConfig, systemConfig) {
     ...
     */
    async findBestMatchingBaseline(testType, testConfig, options) {
        ...
        candidates.sort((a, b) => {
            ...
            let scoreB = 0;
            // Prefer same version
            if (options.version && a.version === options.version)
                scoreA += 100;
            if (options.version && b.version === options.version)
                scoreB += 100;
     ...
     */
    calculateComparison(current, baseline) {
     ...
     */
    calculateOverallScore(differences) {
        let score = 100;
     ...
     */
    convertBaselinesToCSV(baselines) {
        const headers = [
...
export async function recordPerformanceBaseline(manager, testType, testName, metrics, options = {}) {
    const testConfig = {
        name: testName,
...
export async function checkForRegression(manager, testType, testName, currentMetrics, options = {}) {
    const testConfig = {
        name: testName,
