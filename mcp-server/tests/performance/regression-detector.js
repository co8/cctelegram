...
export class RegressionDetector extends EventEmitter {
    ...
    alertsPath;
    constructor(baselineManager, config = {}) {
        super();
     ...
     */
    async checkRegression(testType, testName, currentMetrics, options = {}) {
        const comparison = await this.baselineManager.compareToBaseline(testType, { name: testName, description: '', parameters: {}, duration: 0, concurrency: 1 }, currentMetrics, options);
     ...
     */
    async analyzeTrends(testType, options = {}) {
        ...
        if (options.alertOnNegativeTrends && risks.length > 0) {
            const trendAlert = {
                ...
                comparison: {}, // Trend alerts don't have comparisons
     ...
     */
    async generateAlertReport(outputPath, options = {}) {
        const format = options.format || 'html';
        let alerts = Array.from(this.alertHistory.values()).flat();
        // Apply filters
        if (options.timeRange) {
            alerts = alerts.filter(a => a.timestamp >= options.timeRange.start &&
    ...
    setupEventListeners() {
        ...
        this.baselineManager.on('comparisonCompleted', async (comparison) => {
            if (comparison.regressionDetected) {
                const severity = this.determineSeverity(comparison.overallScore);
                const alert = await this.createAlert(comparison.current.testType, comparison.current.testConfiguration.name, comparison, severity);
     ...
     */
    async createAlert(testType, testName, comparison, severity) {
        const alert = {
     ...
     * Auto-acknowledge old alerts
     */
    async autoAcknowledgeOldAlerts() {
     ...
     */
    generateAlertReportHTML(alerts) {
...
export async function setupRegressionDetection(baselineManager, config = {}) {
    const detector = new RegressionDetector(baselineManager, config);
    await detector.initialize();
    return detector;
}
export async function quickRegressionCheck(detector, testType, testName, metrics) {
