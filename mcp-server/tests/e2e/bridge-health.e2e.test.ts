...
test('Health endpoint should respond with valid status', async () => {
  try {
    ...
    expect(response.data).toHaveProperty('events_processed');
    expect(response.data).toHaveProperty('cpu_usage_percent');
    expect(response.data).toHaveProperty('memory_usage_mb');
    
    // Validate response structure
    expect(typeof response.data.status).toBe('string');
    expect(typeof response.data.timestamp).toBe('string');
    expect(typeof response.data.uptime_seconds).toBe('number');
    expect(typeof response.data.events_processed).toBe('number');
    expect(typeof response.data.cpu_usage_percent).toBe('number');
    expect(typeof response.data.memory_usage_mb).toBe('number');
...
test('Metrics endpoint should provide performance data', async () => {
  try {
    ...
    expect(response.data).toContain('cc_telegram_cpu_usage_percent');
    expect(response.data).toContain('cc_telegram_memory_usage_bytes');
    expect(response.data).toContain('cc_telegram_events_processed_total');
    expect(response.data).toContain('cc_telegram_messages_sent_total');
    
    // Validate that metrics contain actual values
    const lines = response.data.split('\n');
    const metricLines = lines.filter(line => line.match(/^cc_telegram_\w+\s+[\d.]+$/));
