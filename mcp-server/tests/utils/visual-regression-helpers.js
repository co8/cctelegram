...
export class VisualRegressionHelpers {
    ...
    baselineDir;
    diffDir;
    threshold = 0.2; // Default threshold for pixel differences
    constructor(screenshotDir, threshold = 0.2) {
     ...
     */
    async captureBaseline(page, name, options = {}) {
     ...
     */
    async captureComparison(page, name, options = {}) {
        const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
        const filename = `${name}-${timestamp}.png`;
        const filepath = path.join(this.screenshotDir, filename);
        const screenshotOptions = {
     ...
     */
    async captureWorkflowScreenshot(page, workflowName, step) {
     ...
     */
    async compareScreenshots(baselinePath, currentPath, threshold = this.threshold) {
        ...
        // Compare images
        const pixelDifference = pixelmatch(baseline.data, current.data, diff.data, width, height, {
            threshold,
            includeAA: false,
            alpha: 0.1,
     ...
     */
    async runVisualTest(page, testName, options = {}) {
        ...
        if (await fs.pathExists(baselinePath)) {
            ...
            result = await this.compareScreenshots(baselinePath, currentPath, options.threshold || this.threshold);
        }
        else {
            // Create new baseline
     ...
     */
    async generateVisualReport(tests) {
        const reportData = {
            timestamp: new Date().toISOString(),
            summary: {
                total: tests.length,
